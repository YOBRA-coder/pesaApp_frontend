...
const DEMO_BUG = true; // toggle ON for recording
const displayMult = DEMO_BUG ? 1 : mult;
 useEffect(() => {
  if (!DEMO_BUG) return;

  const id = setInterval(() => {
    setCountdown(c => c); // useless update → forces re-render
  }, 100);
        ...
        <div className="w-full lg:w-64 flex flex-col gap-3 shrink-0">
          ...
          {phase !== 'waiting' && (
            <div className="card text-center border"
              ...
              <p className={clsx('font-display font-black text-5xl', mc)}>{displayMult.toFixed(2)}x</p>
            </div>
          )}
